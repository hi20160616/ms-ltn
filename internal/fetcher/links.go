...
func fetchLinks() ([]string, error) {
	...
	for _, rawurl := range configs.Data.MS["ltn"].URL {
	...
	rt = linksFilter(rt, `https://news.*/news/.*`)
	rt = kickOutLinksMatchPath(rt, "/life/")
	rt = kickOutLinksMatchPath(rt, "/society/")
	rt = kickOutLinksMatchPath(rt, "/novelty/")
	rt = kickOutLinksMatchPath(rt, "/local/")
	return rt, nil
}
...
func getLinks(rawurl string) ([]string, error) {
	...
	if links, err := exhtml.ExtractLinks(u.String()); err != nil {
		return nil, errors.WithMessagef(err, "[%s] cannot extract links from %s",
			configs.Data.MS["ltn"].Title, rawurl)
	} else {
